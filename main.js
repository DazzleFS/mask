...
classifier = ml5.imageClassifier("https://teachablemachine.withgoogle.com/models/8J5Q1IeHz/model.json ",modelLoaded);
...
function gotResult(error, results) {
    if (error) {
       ...
       if(results[0].label == "Superb")
        ...
        {
            document.getElementById("status").innerHTML = "Thumbs Up";
        }
       if(results[1].label == "Superb")
        {
            document.getElementById("status").innerHTML = "Super";
        }
        if(results[1].label == "Swag")
        {
            document.getElementById("status").innerHTML = "Swagging";
        }
        if(results[1].label == "Thumbs Down")
        {
            document.getElementById("update_status").innerHTML = "Bad";
        }
